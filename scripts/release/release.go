...
var versionLine = regexp.MustCompile(`(?m)^const GOMEGA_VERSION = "([^"]*)"$`)
var semver = regexp.MustCompile(`^(\d+)\.(\d+)\.(\d+)$`)

// CurrentVersion reads GOMEGA_VERSION out of gomega_dsl.go's source.
func CurrentVersion(versionGo string) (string, error) {
	matches := versionLine.FindAllStringSubmatch(versionGo, -1)
	if len(matches) != 1 {
		return "", fmt.Errorf("expected exactly one `const GOMEGA_VERSION = \"X.Y.Z\"` line in %s, found %d", versionPath, len(matches))
	}
...
func NextVersion(current string, bump string) (string, error) {
	...
	if parts == nil {
		return "", fmt.Errorf("GOMEGA_VERSION %q is not a plain X.Y.Z version", current)
	}
...
// SetVersion rewrites the GOMEGA_VERSION line in gomega_dsl.go's source, leaving every other byte alone.
func SetVersion(versionGo string, version string) (string, error) {
	if _, err := CurrentVersion(versionGo); err != nil {
		return "", err
	}
	return versionLine.ReplaceAllLiteralString(versionGo, `const GOMEGA_VERSION = "`+version+`"`), nil
}
